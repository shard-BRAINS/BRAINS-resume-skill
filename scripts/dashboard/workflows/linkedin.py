...
def render(file_path: Optional[Path] = None, key_prefix: str = "linkedin") -> None:
    ...
    zip_path_str = st.text_input("Path to LinkedIn export ZIP", key=f"{key_prefix}_zip_path", value=str(file_path) if file_path else "")
    if zip_path_str:
        p = Path(zip_path_str)
        if not p.exists():
            st.error(f"File not found: {p}")
        else:
            handoff_button("linkedin", [p], note="Claude Code will ingest and summarise the export.", key=f"{key_prefix}_btn")
